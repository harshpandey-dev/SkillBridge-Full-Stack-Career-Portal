...
export default function Register({ onLogin, navigate }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    university: '', major: '', company: '', position: '', terms: false,
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validate()) return
    setLoading(true)
    const email = form.email || `${form.firstName.toLowerCase()}.${form.lastName.toLowerCase()}@${role === 'student' ? 'university.edu' : 'company.com'}`
    setTimeout(() => onLogin(email, role), 800)
  }
                ...
                {role === 'student' ? (
                  <>
                    <div>
                      ...
                      <input value={form.university} onChange={e => set('university', e.target.value)} placeholder="Stanford University" className={`w-full border rounded px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#2563EB]/10 ${errors.university ? 'border-[#DC2626]' : 'border-[#E4E7EC] focus:border-[#2563EB]'}`} />
                    ...
                    <div>
                      ...
                      <input value={form.major} onChange={e => set('major', e.target.value)} placeholder="Computer Science" className="w-full border border-[#E4E7EC] rounded px-3 py-2.5 text-sm outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#172033] mb-1.5">Expected graduation</label>
                      <select defaultValue="May 2026" className="w-full border border-[#E4E7EC] rounded px-3 py-2.5 text-sm text-[#172033] outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10">
                  ...
                  <>
                    <div>
                      ...
                      <input value={form.company} onChange={e => set('company', e.target.value)} placeholder="Stripe, Inc." className={`w-full border rounded px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#2563EB]/10 ${errors.company ? 'border-[#DC2626]' : 'border-[#E4E7EC] focus:border-[#2563EB]'}`} />
                      {errors.company && <p className="text-xs text-[#DC2626] mt-1">{errors.company}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#172033] mb-1.5">Your role</label>
                      <input value={form.position} onChange={e => set('position', e.target.value)} placeholder="Senior Technical Recruiter" className="w-full border border-[#E4E7EC] rounded px-3 py-2.5 text-sm outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[#172033] mb-1.5">Company size</label>
                      <select defaultValue="1,001–10,000 employees" className="w-full border border-[#E4E7EC] rounded px-3 py-2.5 text-sm text-[#172033] outline-none">
                        <option>1–50 employees</option>
                        <option>51–200 employees</option>
                        <option>201–1,000 employees</option>
                        <option>1,001–10,000 employees</option>
